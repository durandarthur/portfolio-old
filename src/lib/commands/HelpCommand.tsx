...
export default class HelpCommand extends Command {
    ...
    render(): string | JSX.Element {
        ...
        const descriptionList = [
            'Affiche ma biographie.',
            'Envoie vers un formulaire de contact.',
            'Nettoie le terminal.',
            'Liste les commandes disponibles.',
            'Affiche mes projets terminés.',
            'Liste de mes liens sociaux.',
            'Passe vers le site portfolio complet.',
            "Listes les technologies que j'utilise.",
            'Affiche mon parcours scolaire.',
        ];
        return (
            <div>
                <div className='mt-3'>
                    ...
                    <ul className='flex flex-col'>
                        {commandList.map((command, i) => (
                            <li key={command}>
                                <span className='underline leading-loose'>{command}</span>
                                {'\n' + descriptionList[i] + '\n'}
                            </li>
                        ))}
                    </ul>
